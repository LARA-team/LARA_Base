...
/**
 * The {@link LPreprocessFactory} class provides immutable (once assigned, the
 ...
 * 
 */
public final class LPreprocessFactory<A extends LaraAgent<A, BO>, BO extends LaraBehaviouralOption<? super A, BO>>
	...
	private Map<LaraDecisionConfiguration, LaraDecisionModeSelector<A>> selectorMap;
	...
	public LPreprocessFactory(LPreprocessorConfigurator<A, BO> configuration) {
		...
		selectorMap = new HashMap<LaraDecisionConfiguration, LaraDecisionModeSelector<A>>();
		...
		selectorMap
				.put(null, this.configuration.DEFAULT_DECISION_MODE_SELECTOR);
		...
		copyComponentMap(selectorMap,
				configuration.getMap(LaraDecisionModeSelector.class),
				LaraDecisionModeSelector.class);
		copyComponentMap(collectorMap,
				configuration.getMap(LaraBOCollector.class),
				LaraBOCollector.class);
		copyComponentMap(preSelectorMap,
				configuration.getMap(LaraBOPreselector.class),
				LaraBOPreselector.class);
		copyComponentMap(updaterMap,
				configuration.getMap(LaraBOUtilityUpdaterBuilder.class),
				LaraBOUtilityUpdaterBuilder.class);
		copyComponentMap(prefUpdaterMap,
				configuration.getMap(LaraPreferenceUpdater.class),
	...
	/**
	 * This method returns a {@link LaraPreprocessorConfiguration} that equals
	...
	public LaraPreprocessorConfiguration<A, BO> getAlteredConfiguration(
		...
		LaraPreprocessorConfiguration<A, BO> dConfiguration = LPreprocessorConfigurator
				.<A, BO> getDefaultPreprocessConfigurator();

		fillConfiguration(changeConfiguration, dConfiguration, selectorMap,
				LaraDecisionModeSelector.class);
		fillConfiguration(changeConfiguration, dConfiguration, collectorMap,
				LaraBOCollector.class);
		fillConfiguration(changeConfiguration, dConfiguration, preSelectorMap,
				LaraBOPreselector.class);
		fillConfiguration(changeConfiguration, dConfiguration, updaterMap,
				LaraBOUtilityUpdaterBuilder.class);
		fillConfiguration(changeConfiguration, dConfiguration, prefUpdaterMap,
				LaraPreferenceUpdater.class);
	...
	public LaraPreprocessor<A, BO> getPreprocessor(
		...
		return new LPreprocessor<A, BO>(
				selectorMap.containsKey(dConfiguration) ? selectorMap.get(dConfiguration)
						: selectorMap.get(null),
				collectorMap.containsKey(dConfiguration) ? collectorMap
						.get(dConfiguration) : collectorMap.get(null),
				preSelectorMap.containsKey(dConfiguration) ? preSelectorMap
						.get(dConfiguration) : preSelectorMap.get(null),
				updaterMap.containsKey(dConfiguration) ? updaterMap.get(
						dConfiguration).getBOUtilityUpdater() : updaterMap.get(
						null).getBOUtilityUpdater(),
				prefUpdaterMap.containsKey(dConfiguration) ? prefUpdaterMap
						.get(dConfiguration) : prefUpdaterMap.get(null),
				dConfiguration);
	}

	/**
	 * Checks whether this preprocessor builder1 conforms to the configuration
	 * represented by the given {@link LaraPreprocessorConfiguration}.
	 * 
	 * @param configuration
	...
	public boolean meetsConfiguration(
		...
		if (!meetsConfiguration(
				configuration.getMap(LaraDecisionModeSelector.class),
				selectorMap, this.configuration.DEFAULT_DECISION_MODE_SELECTOR)) {
			return false;
		}
		if (!meetsConfiguration(configuration.getMap(LaraBOCollector.class),
				collectorMap, this.configuration.DEFAULT_BO_COLLECTOR)) {
			return false;
		}
		if (!meetsConfiguration(configuration.getMap(LaraBOPreselector.class),
				preSelectorMap, this.configuration.DEFAULT_BO_PRESELECTOR)) {
			return false;
		}
		if (!meetsConfiguration(
				configuration.getMap(LaraBOUtilityUpdaterBuilder.class),
				updaterMap, this.configuration.DEFAULT_BO_UPDATE_BUILDER)) {
			return false;
		}
		if (!meetsConfiguration(
				configuration.getMap(LaraPreferenceUpdater.class),
				prefUpdaterMap, this.configuration.DEFAULT_PREFERENCE_UPDATER)) {
			return false;
		}
	...
	private void copyComponentMap(Map instanceMap,
			Map<LaraDecisionConfiguration, ?> configuratorMap, Class s) {
		for (Map.Entry<LaraDecisionConfiguration, ?> entry : configuratorMap
				.entrySet()) {
	...
	private <T extends LaraPreprocessorComp<A>, U extends LaraPreprocessorComp<?>> void fillConfiguration(
			LaraPreprocessorConfiguration<A, BO> changeConfiguration,
			LaraPreprocessorConfiguration<A, BO> dConfiguration,
		...
		for (Map.Entry<LaraDecisionConfiguration, T> entry : map.entrySet()) {
			if (!(changeConfiguration.get(entry.getKey(), clazz) == null)) {
				changeConfiguration.set(entry.getKey(), clazz,
						(T) changeConfiguration.get(entry.getKey(), clazz));
			} else {
				changeConfiguration
	 ...
	 *         else Created by Sascha Holzhauer on 08.02.2010
	 */

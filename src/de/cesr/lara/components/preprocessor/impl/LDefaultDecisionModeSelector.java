...
public class LDefaultDecisionModeSelector<A extends LaraAgent<A, BO>, BO extends LaraBehaviouralOption<?, ? extends BO>>
	...
	static private Logger logger = Log4jLogger
			.getLogger(LDefaultDecisionModeSelector.class);
	...
	public void onInternalEvent(LaraEvent e) {
		...
		if (!customIsDelibaterive(event)
			...
			BO bo = (BO) agent.getLaraComp().getGeneralMemory()
		...
		if (useDeliberative) {
			...
			agent.getLaraComp().getDecisionData(dConfig)
			...
			@SuppressWarnings("unchecked")
			// unchecked cast
			LaraDeciderFactory<A, BO> factory = LHabitDeciderFactory
					.getFactory(agent.getClass());
			agent.getLaraComp().getDecisionData(dConfig)
					.setDeciderFactory(factory);
		}

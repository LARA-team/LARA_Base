 ...
 * 
 */
public class LPseudoPrefereceUpdater<A extends LaraAgent<? super A, ?>> implements LaraPreferenceUpdater<A> {
	...
	public void updatePreferenceWeights(A agent, LaraDecisionConfiguration dConfiguration) {
		agent.getLaraComp().getDecisionData(dConfiguration).setSituationalPreferences(agent.getLaraComp().getPreferenceWeights());
	}

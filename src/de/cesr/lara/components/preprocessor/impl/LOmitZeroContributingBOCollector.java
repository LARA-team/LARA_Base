...
/**
 * This LaraBoScanner implementation also filters out behavioural option that indeed define one or more
 * preferenceWeights also defined in the {@link LaraDecisionConfiguration} but whose utility is not above zero. This
 * might be critical for some models since the utility may increase during updating but might be useful for others.
 * 
 * @param <A>
 * @param <BO>
	...
	public Collection<BO> collectBOs(A agent, LaraBOMemory<BO> bomemory, LaraDecisionConfiguration dConfiguration) {
		...
		for (BO bo : bomemory.recallAllMostRecent()) {
			...
			for (Entry<Class<? extends LaraPreference>, Double> utility : bo.getValue().entrySet()) {
				if (utility.getValue().doubleValue() > 0.0 && dConfiguration.getPreferences().contains(utility.getKey())) {
					contributes = true;
					break;
				}
			}

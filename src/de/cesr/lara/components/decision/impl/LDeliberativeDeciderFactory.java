...
/**
 * TODO consistent naming schema factory or builder TODO derive an
 * AbstractLDeciderFactory that implements the singleton!
 * 
 * @param <A>
 *            type of agent
 * @param <BO>
 ...
 */
public class LDeliberativeDeciderFactory<A extends LaraAgent<A, BO>, BO extends LaraBehaviouralOption<? super A, BO>>
	...
	public LaraDecider<BO> getDecider(A agent,
		...
		if (bos != null) {
			...
			LaraDeliberativeDecider<BO> decision = new LDeliberativeDecider<BO>(
					dConfiguration);
			decision.setSelectableBOs(agent.getLaraComp()
					.getDecisionData(dConfiguration).getBos());

			if (dConfiguration.getHeuristic() == null) {
				throw new IllegalStateException(
						"Heuristic component was not set in decision builder");
			}
			decision.setDeliberativeChoiceComponent(dConfiguration
					.getHeuristic());
			...
			if (dConfiguration.getPreferences() == null) {
				logger.error("No preferenceWeights set for LaraDecisionConfiguration "
						+ dConfiguration);
			}
			...
			for (Class<? extends LaraPreference> goal : dConfiguration
					.getPreferences()) {
				prefs.put(goal,
						agent.getLaraComp().getPreferenceWeights().get(goal));
			}
